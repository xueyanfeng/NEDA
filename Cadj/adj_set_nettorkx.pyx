...
def SI_sampling(G,seed_node,sample_number,is_copy):
    ...
    for j in range(sample_number):
        ...
        if np.array(prob).sum():
            p = np.array(prob).astype(np.float64)
            p /= p.sum()
        else:   # The similarity between the source node and the target node is 0
            # print("seed node:{}".format(seed_node))
...
def get_adj_set_nettorkx(G,infected_number,sim,is_copy):
    ...
    if sim:
        SS = SI_sampling(G, 3, infected_number,is_copy)  #TO test
...
def get_clustering_coefficient(G,adj_lists):
    cc_dict = {}
    for node,neigh in adj_lists.items():
        cc_dict[node] = nx.clustering(G.subgraph([node] + list(neigh)),node)
    return cc_dict
